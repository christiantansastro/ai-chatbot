...
async function generateClientReportContent({
  ...
  try {
    ...
    if (includeCommunicationHistory) {
      ...
      // Only select the actual fields that exist in communications table
      let query = supabase
        .from('communications')
      ...
      if (communicationsError) {
        ...
        // Continue without communications if error
        communications = [];
      } else {
      ...
      console.log(`📄 CLIENT REPORT TOOL: Found ${communications.length} communications`);
    }

    // Get financial data
    console.log('📄 CLIENT REPORT TOOL: Getting financial data...');
    ...
    try {
      ...
      if (financialsError) {
        console.warn('⚠️ CLIENT REPORT TOOL: Could not get financial data:', financialsError);
        // Continue without financial data if error
        financialData = [];
      } else {
      ...
      console.log(`📄 CLIENT REPORT TOOL: Found ${financialData.length} financial records`);
      ...
      console.warn('⚠️ CLIENT REPORT TOOL: Error processing financial data:', financialError);
...
export const createClientReport = ({ session, dataStream }: CreateClientReportProps) =>
  tool({
    description: "Generate a comprehensive client report document containing client information, financial summary, and communication history.",
