...
function formatSearchResults(results: any[], limit: number, method: string, searchId: string): SearchResult {
 ...
 const formattedResults = limitedResults.map((client: any, index: number) => ({
   ...
   contact1Phone: client.contact_1_phone || 'Not provided',
   contact2: client.contact_2 || 'Not provided',
   relationship2: client.relationship_2 || 'Not provided',
   contact2Phone: client.contact_2_phone || 'Not provided',
   // Case details
   notes: client.notes || 'No notes',
   county: client.county || 'Not provided',
   courtDate: client.court_date ? new Date(client.court_date).toLocaleDateString() : 'Not provided',
   quoted: client.quoted || 'Not provided',
   initialPayment: client.initial_payment || 'Not provided',
   dueDateBalance: client.due_date_balance ? new Date(client.due_date_balance).toLocaleDateString() : 'Not provided',
   ...
   incarcerationLocation: client.incarceration_location || 'Not provided',
   incarcerationReason: client.incarceration_reason || 'Not provided',
   lastBondHearingDate: client.last_bond_hearing_date ? new Date(client.last_bond_hearing_date).toLocaleDateString() : 'Not provided',
   lastBondHearingLocation: client.last_bond_hearing_location || 'Not provided',
   dateOfIncident: client.date_of_incident ? new Date(client.date_of_incident).toLocaleDateString() : 'Not provided',
   incidentCounty: client.incident_county || 'Not provided',
   onProbation: client.on_probation !== undefined ? (client.on_probation ? 'Yes' : 'No') : 'Not specified',
   probationCounty: client.probation_county || 'Not provided',
   probationOfficer: client.probation_officer || 'Not provided',
   probationTimeLeft: client.probation_time_left || 'Not provided',
   onParole: client.on_parole !== undefined ? (client.on_parole ? 'Yes' : 'No') : 'Not specified',
   paroleOfficer: client.parole_officer || 'Not provided',
   paroleTimeLeft: client.parole_time_left || 'Not provided',
   arrestReason: client.arrest_reason || 'Not provided',
   charges: client.charges || 'Not provided',
   // Civil-specific fields
   servedPapersOrInitialFiling: client.served_papers_or_initial_filing || 'Not provided',
   caseType: client.case_type || 'Not provided',
   otherSideName: client.other_side_name || 'Not provided',
   otherSideRelation: client.other_side_relation || 'Not provided',
   otherSideContactInfo: client.other_side_contact_info || 'Not provided',
   otherSideAttorneyInfo: client.other_side_attorney_info || 'Not provided',
   childrenInvolved: client.children_involved !== undefined ? (client.children_involved ? 'Yes' : 'No') : 'Not specified',
   childrenDetails: client.children_details || 'Not provided',
   previousCourtOrders: client.previous_court_orders !== undefined ? (client.previous_court_orders ? 'Yes' : 'No') : 'Not specified',
   previousOrdersCounty: client.previous_orders_county || 'Not provided',
   previousOrdersCaseNumber: client.previous_orders_case_number || 'Not provided',
   // Metadata
