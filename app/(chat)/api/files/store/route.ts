...
import { validateClientForFileStorage } from "@/lib/utils/client-validation";
...
export async function POST(request: Request) {
  try {
    ...
    const { tempId, filename, contentType, size, fileBuffer, clientName } =
      validatedData.data;
    ...
    if (requestedClientName) {
      try {
        const validation =
          await validateClientForFileStorage(requestedClientName);

        if (validation.isValid) {
          const resolvedClientName =
            validation.clientName ?? requestedClientName;
    ...
    const storedClientName =
      validatedClientName || requestedClientName || "Unassigned";
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      console.error("Missing Supabase environment variables for file storage");
      return NextResponse.json(
    ...
    console.log(
      "📁 STORE DEBUG: Converting base64 to buffer, length:",
      fileBuffer.length
    );
    const fileBufferBinary = Buffer.from(fileBuffer, "base64");
    console.log(
      "📁 STORE DEBUG: Buffer converted, binary length:",
      fileBufferBinary.length
    );

    // Upload file to Supabase Storage
    console.log(
      `📁 STORE DEBUG: Uploading to storage path: uploads/${filename}`
    );
    ...
    if (uploadError) {
      console.error(
        "📁 STORE DEBUG: Supabase Storage upload error:",
        uploadError
      );
      return NextResponse.json(
        {
